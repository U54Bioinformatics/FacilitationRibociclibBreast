# Step three for newMCF7 cells
# pdf('Figs/step3m.pdf')
tparms <- NULL
nm <- "newMCF7"
Mnms <- paste0("M",1:4)  #M5 has the same values for S and R
# Consensus values
alphaRSval <- 0.0
alphaSRval <- 2.0
for (envm in Mnms) {
...
# Estimate alphas
  ...
  if (plotm==1) {
    ...
    plot(cells ~ day,tmpSM0,pch=19,ylim=range(tmpSM0$cells,tmpRM0$cells),
         col=mcf7cols["S"],xlab="Day",ylab="Cells",cex.lab=1.3,cex.axis=1.3)
    points(cells ~ day,tmpRM0,pch=19,col=mcf7cols["R"])
    lines(pred ~ day,tmpSM,subset=rep==1,pch=19,lwd=3,col=mcf7cols["S"])
    lines(pred ~ day,tmpRM,subset=rep==1,pch=19,lwd=3,col=mcf7cols["R"])
    lines(testpred ~ day,tmpSM,subset=rep==1,pch=19,lwd=3,lty=2,col=mcf7cols["S"])
    lines(testpred ~ day,tmpRM,subset=rep==1,pch=19,lwd=3,lty=2,col=mcf7cols["R"])
...
# Assign to its name and just overwrite newMCF7.rK
  assign(paste0(nm,".rK"),tfile.rK)
  newMCF7.rK[newMCF7.rK$file==nm,] <- tfile.rK
}
# dev.off()
