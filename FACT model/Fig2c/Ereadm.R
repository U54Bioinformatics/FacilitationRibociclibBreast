...
Sc <- read.csv("../DATA/SEcountsF.csv",header=FALSE)
Rc <- read.csv("../DATA/REcountsF.csv",header=FALSE)
Sc[,1] <- days
Rc[,1] <- days
...
tmp <- c("day",rep("R000E000",9),
               rep("R200E000",9))
tmp2 <- c("",rep(c(rep(".S",3),rep(".M",3),rep(".R",3)),2))
tmp3 <- c("",rep(1:3,6))
...
# Put it together
EcountsF <- rbind(allSc,allRc)
EcountsF <- cbind(file="F",EcountsF)
...
#  EcountsFGF 
days <- c(0, 4, 7, 11, 14, 18, 21)
Sc <- read.csv("../DATA/SEcountsFGF.csv",header=FALSE)
Rc <- read.csv("../DATA/REcountsFGF.csv",header=FALSE)
Sc[,1] <- days
Rc[,1] <- days
...
# Put it together
EcountsFGF <- rbind(allSc,allRc)
EcountsFGF <- cbind(file="FGF",EcountsFGF)
...
#  EcountsM 
days <- c(0, 4, 7, 11, 14, 18, 21)
Sc <- read.csv("../DATA/SEcountsM.csv",header=FALSE)
Rc <- read.csv("../DATA/REcountsM.csv",header=FALSE)
Sc[,1] <- days
Rc[,1] <- days
...
# Put it together
EcountsM <- rbind(allSc,allRc)
EcountsM <- cbind(file="M",EcountsM)
