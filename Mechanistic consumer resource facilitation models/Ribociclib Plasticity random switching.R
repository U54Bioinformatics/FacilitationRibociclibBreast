...
lu_table<-data.table(Experiment_Data[Day==0]%>%group_by(Resistance, DoseNum)%>%mutate(S=mean(SCellNum),R=mean(RCellNum),"Sp"=0,"Rp"=0))
...
Info <- data.table(gather(lu_table%>% dplyr::select( S,R, Sp,Rp, DoseNum, Replicate,experim_unit,Compostition),State,Initial_Condition, -c(DoseNum,Replicate,experim_unit,Compostition)) )
Info$State <- factor(Info$State, levels = c("S","R","Sp","Rp"))
Info <- Info[order(State,experim_unit)]
Info[ ,ode_ID:=1:nrow(Info)]

# ode initial conditions and indexing
inits <- y0 <- c("S"=lu_table$S,  "R"=lu_table$R,
                 "Sp"=rep(0,length(lu_table$S)) ,
                 "Rp"=rep(0,length(lu_table$R)) )
...
index_S <- grep("S",names(y0))[!grep("S",names(y0))%in%c(grep("Sp",names(y0)),grep("Sp",names(y0)) )]
index_R <- grep("R",names(y0))[!grep("R",names(y0))%in%c(grep("Rp",names(y0)),grep("Rp",names(y0)) )]
index_Sp <- grep("Sp",names(y0))
index_Rp <- grep("Rp",names(y0))
...
plot(lu_table$DoseNum,Info[State=="S"]$DoseNum);         if(!all(lu_table$DoseNum==Info[State=="S"]$DoseNum)){print("DANGER:: Wrong data entry. \n dose data not being parsed correctly to stan. \n mismatch identified")}
...
fit.dd[,State:="R"]
fit.dd[StateOld=="SCellNum",State:="S"]
...
x_iSetts  <- 1:length(index_S)   # index for each populations      #x_rSetts  <- c(Info$dose)        # drug doses of each population
...
real R; real Rp;    real S; real Sp;  real doses;
real r0_R;real rp0_R;real r0_S;real rp0_S;
real B_R;real Bp_R;real B_S;real Bp_S;
real K_R;real Kp_R;real K_S;real Kp_S;
real delta_R;real deltap_R;real delta_S;real deltap_S;
real gamma_R;real gamma_S;
real dydt[ 4*size(x_i) ]; int s_i;  
real RC; 
s_i = size(x_i);
r0_R = params[1]; rp0_R = params[2];r0_S = params[3]; rp0_S = params[4]; 
B_R = params[5]; Bp_R = params[6]; B_S = params[7]; Bp_S = params[8];
K_R = params[9]; Kp_R = params[10]; K_S = params[11]; Kp_S = params[12];
delta_R = params[13];deltap_R = params[14];delta_S = params[15];deltap_S = params[16];
gamma_R = params[17];gamma_S = params[18];

// Rate of change functions: dy/dt
for(i in 1:s_i){ 
R = y[i];   Rp = y[( s_i + i )]; S = y[( 2*s_i + i )]; Sp = y[( 3*s_i + i )];
doses = x_r[i];
RC = (1 - R/K_R - Rp/Kp_R - S/K_S - Sp/Kp_S);  // Resource limitation
dydt[i] = (r0_R/(1+B_R*doses)) *RC*R - delta_R*R - gamma_R*R;
dydt[(s_i + i)] = (rp0_R/(1+Bp_R*doses)) *RC*Rp - deltap_R*Rp + gamma_R*R;
dydt[(2*s_i + i)] = (r0_S/(1+B_S*doses))*RC*S - delta_S*S - gamma_S*S;
dydt[(3*s_i + i)] = (rp0_S/(1+Bp_S*doses))*RC*Sp - deltap_S*Sp + gamma_S*S;
}
return dydt;
}
...
parameters {
real r0_R_raw;real rp0_R_raw;real r0_S_raw;real rp0_S_raw;
real B_R_raw;real Bp_R_raw;real B_S_raw;real Bp_S_raw;
real K_R_raw;real Kp_R_raw;real K_S_raw;real Kp_S_raw;
real delta_R_raw;real deltap_R_raw;real delta_S_raw;real deltap_S_raw;
real gamma_R_raw;real gamma_S_raw;
real sigma_raw;

}
transformed parameters{
real<lower=0> r0_R;real<lower=0> rp0_R;real<lower=0> r0_S;real<lower=0> rp0_S;
real<lower=0> B_R;real<lower=0> Bp_R;real<lower=0> B_S;real<lower=0> Bp_S;
real<lower=0> K_R;real<lower=0> Kp_R;real<lower=0> K_S;real<lower=0> Kp_S;
real<lower=0> delta_R;real<lower=0> deltap_R;real<lower=0> delta_S;real<lower=0> deltap_S;
real<lower=0> gamma_R;real<lower=0> gamma_S;

real<lower=0> sigma; real params [18];          // parameters need to be passed to ODE integrator as a real vector
...
r0_S= exp(r0_S_raw);
rp0_S= exp(rp0_S_raw);
K_R= 1 + exp(10 + K_R_raw);
Kp_R= 1 + exp(10 + Kp_R_raw);
K_S= 1 + exp(10 + K_S_raw);
Kp_S= 1 + exp(10 + Kp_S_raw);
B_R= exp(B_R_raw-3);
Bp_R= exp(Bp_R_raw-3);
B_S= exp(B_S_raw-3);
Bp_S= exp(Bp_S_raw-3);
delta_R= exp(delta_R_raw-3);
deltap_R= exp(deltap_R_raw-3);
delta_S= exp(delta_S_raw-3);
deltap_S= exp(deltap_S_raw-3);
gamma_R= exp(gamma_R_raw); 
gamma_S= exp(gamma_S_raw); 
sigma= exp(sigma_raw);

params[1] = r0_R ; params[2] = rp0_R ; params[3] = r0_S ; params[4] = rp0_S ;
params[5] = B_R ;  params[6] = Bp_R ; params[7] = B_S ; params[8] = Bp_S ; 
params[9] = K_R ; params[10] = Kp_R ; params[11] = K_S ; params[12] = Kp_S ; 
params[13] = delta_R ; params[14] = deltap_R ; params[15] = delta_S ;params[16] = deltap_S ; 
params[17] = gamma_R ;params[18] = gamma_S ;
...
for(i in 1:nt){for(j in 1:nStatesObs){ lnY[i,j] ~ normal( log(y_hat[i,j] + y_hat[i,j+nStatesObs] + 1.0 ) , sigma ); }}//for( j in 1:nStatesObs ){   if ( is_non_zero_state[j]==1 ){  lnY[,j] ~ normal(log( 1.0 + to_row_vector(y_hat[,j]) ) , sigma ) ;   }}                     

// Prior on the rate or proliferation of cells to aid identifiability of other fail pars
r0_R_raw ~ normal(1e-5, 1);rp0_R_raw ~ normal(1e-5, 1);r0_S_raw ~ normal(1e-5, 1);rp0_S_raw ~ normal(1e-5, 1);
K_R ~ normal(1.6e+05, 5e4);  Kp_R ~ normal(1.6e+05, 5e4);K_S ~ normal(1.6e+05, 5e4);  Kp_S ~ normal(1.6e+05, 5e4);
B_R_raw ~ normal(0, 2);Bp_R_raw ~ normal(0, 2); B_S_raw ~ normal(0, 2);Bp_S_raw ~ normal(0, 2);
delta_R_raw ~ normal(0, 2);deltap_R_raw ~ normal(0, 2);delta_S_raw ~ normal(0, 2);deltap_S_raw ~ normal(0, 2);
gamma_R_raw ~ normal(0, 2);gamma_S_raw ~ normal(0, 2);
sigma_raw ~ normal(-3, 10);
}
...
log_lik[j+nStatesObs*(i-1 )] = normal_lpdf(lnY[i,j] |  log(y_hat[i,j] + y_hat[i,j+nStatesObs]  + 1.0 )  , sigma);
}}
...
test0 <- stan( model_code = Cancer_mod, data = stan_data, init=0,
...
save(test,stan_data,fit.dd,Info,doses,doses_seq,experim_unit,Experiment_Data,inits,lu_table,ndose,obs.times,pred_t,times,x_iSetts,y0,
     index_R,index_Rp,index_S,index_Sp,file="/Users/jason/Downloads/Ribo_fit_PlasticityModelRandSwitch.Rdata")
...
stan_dens(test,inc_warmup=T,pars=c("k_N"))
stan_trace(test,inc_warmup=T,pars=c("lp__"))

stan_trace(test,inc_warmup=T,pars=c("gamma_A" ,  "gamma_N" ,"sigma"))

stan_trace(test,inc_warmup=T,pars=c("r_N","k_N","B_N" ,"c_N" ,  "gamma_N" ))
stan_trace(test,inc_warmup=F,pars=c("r_N","k_N","B_N" ,"c_N" ,  "gamma_N" ,"delta_N","lambda_N",
...
pars.out<-data.table(bind_rows(extract(test,pars=c("r0_R","rp0_R","r0_S","rp0_S","B_R" ,"Bp_R" ,"B_S" ,"Bp_S" ,"K_R","Kp_R","K_S","Kp_S","delta_R","deltap_R","delta_S","deltap_S",
                                                   "gamma_R" ,"delta_S","sigma","lp__"))))[lp__ >0]
...
preds2_wide <- data.table( preds2 %>%
                             ...
                             spread(State,value) ) 
preds2_wide[,Ntot:=S + Sp]
preds2_wide[,Atot:=R + Rp]
preds2_wide <- preds2_wide%>%select(-c(R ,Rp , S, Sp))

setnames(preds2_wide,old=c("Ntot", "Atot"),new=c("S", "R"))
...
preds3 <- data.table(gather(preds3_wide, State,value,S : R) )
