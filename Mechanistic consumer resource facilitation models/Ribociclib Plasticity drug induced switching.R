...
dydt[i] = (r0_R/(1+B_R*doses)) *RC*R - delta_R*R - gamma_R*doses*R;
dydt[(s_i + i)] = (rp0_R/(1+Bp_R*doses)) *RC*Rp - deltap_R*Rp + gamma_R*doses*R;
dydt[(2*s_i + i)] = (r0_S/(1+B_S*doses))*RC*S - delta_S*S - gamma_S*doses*S;
dydt[(3*s_i + i)] = (rp0_S/(1+Bp_S*doses))*RC*Sp - deltap_S*Sp + gamma_S*doses*S;
}
return dydt;
}
...
save(test,stan_data,fit.dd,Info,doses,doses_seq,experim_unit,Experiment_Data,inits,lu_table,ndose,obs.times,pred_t,times,x_iSetts,y0,
     index_R,index_Rp,index_S,index_Sp,file="/Users/jason/Downloads/Ribo_fit_PlasticityModel.Rdata")
