...
lu_table<-data.table(Experiment_Data[Day==0]%>%group_by(Resistance, DoseNum)%>%mutate(N=mean(SCellNum),A=mean(RCellNum),"Z_N_"=0,"Z_A_"=0,E=0))
...
Info <- data.table(gather(lu_table%>% dplyr::select( N,A, E,Z_N_,Z_A_, DoseNum, Replicate,experim_unit,Compostition),State,Initial_Condition, -c(DoseNum,Replicate,experim_unit,Compostition)) )
Info$State <- factor(Info$State, levels = c("N","A","Z_N_","Z_A_","E"))
...
index_N <- grep("N_",names(y0))[!grep("N_",names(y0))%in%c(grep("Z_N_",names(y0)) )]
index_A <- grep("A_",names(y0))[!grep("A_",names(y0))%in%c(grep("Z_A_",names(y0)) )]
index_Z_N_ <- grep("Z_N_",names(y0))
index_Z_A_ <- grep("Z_A_",names(y0))


index_E <- grep("E_",names(y0))          ; if( length(unique(Experiment_Data$experim_unit))!= length(inits)/5){ print("DANGER::incorrect number of initial conditions specified")}
...
real N; real A;    real E; real Z_N; real Z_A;   real doses;

real r_N; real B_N; real c_N; real k_N; real gamma_N; real delta_N;  real lambda_N; 
real r_A; real B_A; real c_A; real k_A; real gamma_A; real delta_A;  real lambda_A; 
real delta;
real dydt[ 5*size(x_i) ]; int s_i;  real RC; real q_x_N; real q_x_A; real Gphase_N; real Gphase_A;
...
dydt[i] = ( Gphase_N * (1 - q_x_N) - Gphase_N*q_x_N - lambda_N )*N          ; // (r_N*N/( 1 + B_N*doses ))*(1 + E/(1 + c_N*E))*(1 - N/k_N - A/k_A) - delta_N*N  ;
dydt[(s_i + i)] = ( Gphase_A * (1 - q_x_A) - Gphase_A*q_x_A - lambda_A )*A  ; //(r_A*A/( 1 + B_A*doses ))*(1 + E/(1 + c_A*E))*(1 - N/k_N - A/k_A) - delta_A*A ;
...
dydt[(4*s_i + i)] =  gamma_N*N +  gamma_A*A - delta*E;
}
return dydt;
}
...
real lambda_N_raw;
real lambda_A_raw;
real delta_raw;                                                       //real delta_N_raw;//real delta_A_raw;//real k_N_raw;//real B_N_raw;//real k_A_raw;//real B_A_raw;//real c_N_raw;//real gamma_N_raw;//real c_A_raw;//real gamma_A_raw;
...
real<lower=0> sigma; real params [15];          // parameters need to be passed to ODE integrator as a real vector
...
save(test,stan_data,fit.dd,Info,doses,doses_seq,experim_unit,Experiment_Data,inits,lu_table,ndose,obs.times,pred_t,times,x_iSetts,y0,
     index_N,index_A,index_Z_N_,index_Z_A_,index_E,file="/Users/jason/Downloads/Ribo_fit_LifeHistoryModelSimplified.Rdata")
...
pars.out<-data.table(bind_rows(extract(test,pars=c("r_N","k_N","B_N" ,"c_N" ,  "gamma_N" ,"delta_N","lambda_N",
                                                   "r_A","k_A","B_A" ,"c_A" ,  "gamma_A" ,"delta_A","lambda_A","delta","sigma","lp__"))))[lp__ >0]
...
preds2_wide <- data.table( preds2 %>%
                             ...
                             spread(State,value) ) 
preds2_wide[,Ntot:=N+Z_N]
preds2_wide[,Atot:=A+Z_A]
preds2_wide <- preds2_wide%>%select(-c(A,E,N,Z_A,Z_N))
