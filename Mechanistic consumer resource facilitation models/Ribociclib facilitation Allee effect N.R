...
dydt[i] = (r_N*N/( 1 + B_N*doses ))*(1 + gamma_N*(N+A)/(1 + c_N*(N+A)))*(1 - N/k_N - A/k_A) - delta_N*N  ;
dydt[(s_i + i)] = (r_A*A/( 1 + B_A*doses ))*(1 + gamma_A*(N+A)/(1 + c_A*(N+A)))*(1 - N/k_N - A/k_A) - delta_A*A ;
...
real<lower=0> sigma; real params [12];          // parameters need to be passed to ODE integrator as a real vector
...
#     index_N,index_A,file="/Users/jason/Downloads/Ribo_fit_AlleeModel.Rdata")
...
pars.out<-data.table(bind_rows(extract(test,pars=c("r_N","k_N","B_N" ,"c_N" ,  "gamma_N" ,"delta_N",
                                                   "r_A","k_A","B_A" ,"c_A" ,  "gamma_A" ,"delta_A","sigma","lp__"))))[lp__ >0]
...
preds2_wide[,Ntot:=N]
preds2_wide[,Atot:=A]
preds2_wide <- preds2_wide%>%select(-c(A,N))
