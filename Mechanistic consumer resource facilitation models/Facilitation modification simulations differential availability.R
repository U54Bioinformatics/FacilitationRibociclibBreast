...
#rho <- 1
#alpha <- 2
luinter<-expand.grid(rho=seq(0,1,length=5),alpha= 1# 10^(-3:2)
                     ,phi=1, omega= seq(0,1,length=5))
...
ggplot( data.table(gather(qss,State,y0,  E: qss_E))[State%in%c("TotA","TotN","E","qss_E")] , aes(y=log(1+y0) ,x=time,col=State ,linetype=as.factor(-rho),group=interaction(rho,State),fill=State))+
  geom_line()+facet_grid(~DoseNum)+theme_classic()+theme(aspect.ratio = 1)


ggplot( res[Compostition=="polyculture"][rho%in%c(1)][alpha==1][omega==1][State%in%c("TotA","TotN","E")],aes(y=log(1+y0) ,x=time,col=State ,linetype=as.factor(-rho),group=interaction(rho,State),fill=State))+
  geom_line()+facet_grid(~DoseNum)


ggplot( res[Compostition=="polyculture"][rho%in%c(0.5,1)][alpha==1][omega==1][State%in%c("TotA","TotN","E")],aes(y=log(1+y0) ,x=time,col=State ,linetype=as.factor(-rho),group=interaction(rho,State),fill=State))+
  geom_line()+facet_grid(~DoseNum)

ggplot( res[Compostition=="polyculture"][rho%in%c(0.0,0.5,1)][alpha==1][omega==1][time==max(time)][State%in%c("TotA","TotN","E")],aes(y=log(1+y0) ,x=DoseNum))+
  geom_smooth(data=res[Compostition=="polyculture"][rho%in%c(1)][alpha==1][omega==1][time==max(time)][State%in%c("TotA","TotN","E")],
...
ggplot( res[Compostition=="polyculture"][omega%in%c(0.0,0.5,1)][alpha==1][rho==1][time==max(time)][State%in%c("TotA","TotN","E")],aes(y=log(1+y0) ,x=DoseNum))+
  geom_smooth(data=res[Compostition=="polyculture"][omega%in%c(1)][alpha==1][rho==1][time==max(time)][State%in%c("TotA","TotN","E")],
              se=F,method="gam",formula=y~s(x,k=5),col="black",aes(size=2.5,group=interaction(omega,rho,State)))+ 
  
  geom_smooth(se=F,method="gam",formula=y~s(x,k=5),aes(col=State , fill=State,size=(omega),group=interaction(omega,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+facet_grid(~State)+theme_classic()+
  #geom_line(aes(col=State , fill=State,size=(rho),group=interaction(rho,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+facet_grid()+theme_classic()+
  geom_point(data=fit.dd_tt[Day==21][Compostition=="polyculture"], aes(y=log(1+value) ,x=DoseNum ,fill=State2 ),colour="black",pch=21,size=2.5)+
  scale_color_manual(values=c("green","red","blue"))+  scale_fill_manual(values=c("green","red","blue"))+
  geom_hline(yintercept = log(1+max(y0) ),col="slategrey", linetype="dashed",size=1.5)+theme(aspect.ratio=1)+
...
ggplot( res[Compostition=="polyculture"][omega%in%c(0.0,0.5,1)][alpha==1][rho%in%c(0.0,0.5,1)][time==max(time)][State%in%c("TotA","TotN","E")],aes(y=log(1+y0) ,x=DoseNum))+
  geom_smooth(data=res[Compostition=="polyculture"][omega%in%c(1)][alpha==1][rho%in%c(0.0,0.5,1)][time==max(time)][State%in%c("TotA","TotN","E")],
              se=F,method="gam",formula=y~s(x,k=5),col="black",aes(size=2.5,group=interaction(omega,rho,State)))+ 
  
  geom_smooth(se=F,method="gam",formula=y~s(x,k=5),aes(col=State , fill=State,size=(omega),group=interaction(omega,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+
  facet_grid(rho~State)+theme_classic()+
  #geom_line(aes(col=State , fill=State,size=(rho),group=interaction(rho,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+facet_grid()+theme_classic()+
  geom_point(data=fit.dd_tt[Day==21][Compostition=="polyculture"], aes(y=log(1+value) ,x=DoseNum ,fill=State2 ),colour="black",pch=21,size=2.5)+
  scale_color_manual(values=c("green","red","blue"))+  scale_fill_manual(values=c("green","red","blue"))+
  geom_hline(yintercept = log(1+max(y0) ),col="slategrey", linetype="dashed",size=1.5)+theme(aspect.ratio=1)+
...
ggplot( res[Compostition=="polyculture"][rho%in%c(0.0,0.5,1)][alpha==1][omega==1][time==max(time)][State%in%c("TotA","TotN")],aes(y=log(1+y0) ,x=DoseNum))+
  geom_smooth(data=res[Compostition=="polyculture"][rho%in%c(1)][alpha==1][omega==1][time==max(time)][State%in%c("TotA","TotN")],
              se=F,method="gam",formula=y~s(x,k=5),col="black",aes(size=10,group=interaction(rho,State)))+ 
  
  geom_smooth(se=F,method="gam",formula=y~s(x,k=5),aes(col=State , fill=State,size=(rho),group=interaction(rho,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+
  facet_grid(~State,labeller = labeller(State = cell.labs))+theme_classic(base_size=18)+
  #geom_line(aes(col=State , fill=State,size=(rho),group=interaction(rho,State)))+ scale_size(name="Facilitation (%)", breaks=c(0,0.5,1),labels = 100*(c(0,0.5,1)), range = c(0.5, 1.4))+facet_grid()+theme_classic()+
  geom_point(data=fit.dd_tt[Day==21][Compostition=="polyculture"], aes(y=log(1+value) ,x=DoseNum ,fill=State2 ),colour="black",pch=21,size=2.5)+
  scale_color_manual(values=c("red","green","blue"),guide = FALSE)+ 
  scale_fill_manual(values=c("red","green","blue"),guide = FALSE)+
...
ggplot( res[Compostition=="polyculture"][omega%in%c(0.0,0.5,1)][alpha==1][rho==1][time==max(time)][State%in%c("TotA","TotN")],aes(y=log(1+y0) ,x=DoseNum))+
  geom_smooth(data=res[Compostition=="polyculture"][omega%in%c(1)][alpha==1][rho==1][time==max(time)][State%in%c("TotA","TotN")],
              se=F,method="gam",formula=y~s(x,k=5),col="black",aes(size=10,group=interaction(omega,rho,State)))+ 
